...
class PerformanceTest(TaskSet):
    ...
    def on_locust_init(environment, **Kwargs):
        if isinstance(environment.runner, MasterRunner):
            log.info("启动Master")
            data = taasClient.make_user_queue()
            log.info(f"用户队列初始化完毕: {data}")
    ...
    def do(self):
        ...
        for i in range(20):
            try:
                log.info(f"客户端模拟: {self.userInfo.get('accountId')}")
                self.defaultClient.send_post(self.client, self.userInfo, nacos=nacos)
    ...
    def require(self):
        self.userInfo = taasClient.get_user()
        assert self.userInfo, "获取可用用户失败"
